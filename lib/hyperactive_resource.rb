...
class HyperactiveResource < ActiveResource::Base
  ...
  # associations
    ...
    def from_json(json)
      clear
      humanized_attributes = @base.attributes.keys.inject({}) { |h, attr_name| h.update(attr_name.humanize => attr_name) }
      messages = Array.wrap(Hash.from_json(json)['errors']['error']) rescue []
      messages.each do |message|
        attr_message = humanized_attributes.keys.detect do |attr_name|
          if message[0, attr_name.size + 1] == "#{attr_name} "
            add humanized_attributes[attr_name], message[(attr_name.size + 1)..-1]
  ...
  protected   ##########################################################
    ...
    def encode(opts = {})
      ...
      add_to_xml_for.each {|key| massaged_attributes[key] = attributes[key] if attributes[key] }
  ...
  #  But any changes aren't kept in sync (like ActiveRecord.. mostly)
    ...
    def method_missing(name, *args)
      ...
      if name.to_s =~ /^(.*)_before_type_cast/
        ...
        return self.attribute_before_type_cast(attribute_name)
      end
